...
export async function adminGlobalSearchAction(
  ...
  try {
    ...
    // Format Products
    const products: AdminProductSearchResult[] = rawProducts.map((p) => {
      const resolvedThumb = safeGetImageBase64(p.image);
      ...
      if (p.isVariable && p.variants && p.variants.length > 0) {
        ...
        mappedVariants = p.variants.map((v) => {
          const variantThumb = safeGetImageBase64(v.image);
          const label =
            v.attributes.map((a) => a.value).join(" / ") || `Variant ${v.sku}`;
          return {
            id: v.id,
            sku: v.sku,
    ...
    // Format Customers
    const customers: AdminCustomerSearchResult[] = rawCustomers.map((c) => {
      const resolvedAvatar = safeGetImageBase64(c.avatar);
